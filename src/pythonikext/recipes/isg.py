...
class IconikStorageGatewayRecipe:
    ...
    def calculate_file_md5(file_path: str, chunk_size: int = 8192) -> str:
        ...
        """
        # Use pythonikext's calculate_md5 utility
        return calculate_md5(file_path, chunk_size=chunk_size)
    ...
    def create_asset(
        ...
        mountpoint = get_mountpoint(file_path)
        ...
        if directory_path.startswith(mountpoint):
            directory_path = directory_path[len(mountpoint):].lstrip('/')
        ...
        if not external_id:
            if self.storage_settings.get('filename_is_external_id', False):
                ...
                external_id = f"{directory_path}/{calculate_md5(file_path)}"
        ...
        if not skip_transcoding:
            ...
            if not proxies_exist and not keyframes_exist and not has_transcode_history:
                if self.storage_settings.get('local_proxy_creation', False):
                    ...
                    # In a real implementation, you would handle local proxy
                    # generation here
                    # TODO: Do we use `FilesSpec.create_asset_proxy`? Also, are we
                    #  sure that the "Server-side proxy generation" isn't
                    #  replicating the same functionality of `create_asset_proxy`?
                else:
